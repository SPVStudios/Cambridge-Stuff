...
//        };
        ...
        float[] vertPositions = new float[3*heightmap_width_px*heightmap_height_px];

        for (int row = 0; row < heightmap_height_px; row++) {
            for (int col = 0; col < heightmap_width_px; col++) {
                float x, y, z;
                // Calculate the index into the vertPositions
                int index = ((row*heightmap_height_px) + col)*3;
//  Work out x, y and z coordinates of the vertex
    ...
    public int[] initializeVertexIndices( float[][] heightmap ) {
        ...
        // create int array for indices of the right size
        ...
        for (int row = 0; row < heightmap_height_px - 1; row++) {
            for (int col = 0; col < heightmap_width_px - 1; col++) {
                ...
                index_count = 6* (((heightmap_width_px-1)*row) + col);
...
//  Initialize each normal to (0,1,0) so that valid normals can be found at edges
        ...
        for (int row = 1; row < heightmap_height_px - 1; row++) {
            for (int col = 1; col < heightmap_width_px - 1; col++) {
                    int index = 3*((heightmap_width_px*row)+col);
//  Create Vector3f Tx
                //Vector3f Tx = new Vector3f(2*delta_x,(heightmap[row+1][col] - heightmap[row-1][col])*delta_x,0);
                Vector3f Tx  = new Vector3f(2*delta_x,heightmap[row][col+1] - heightmap[row][col-1],0);
//  Create Vector3f Tz
                //Vector3f Tz = new Vector3f(0,(heightmap[row][col+1] - heightmap[row][col-1])*delta_z,2*delta_z);
...
//  Calculate Vector3f vertNormal by as the normalized cross product of vecNx and vecNz and put in vertNormals
                ...
                vertNormals[index++] = vertNormal.y;
                vertNormals[index] = vertNormal.z;
            }
        }
        return vertNormals;
    }
    ...
    public void loadDataOntoGPU( float[] vertPositions, int[] indices, float[] vertNormals ) {
        ...
        vertexArrayObj = glGenVertexArrays(); // Get a OGL "name" for a vertex-array object
        glBindVertexArray(vertexArrayObj); // Create a new vertex-array object with that name and bind it to the current context
        ...
        int normal_handle = glGenBuffers(); // Get an OGL name for another buffer object
